...
def count(string, num):
    ...
    # Retrieve from the memo at the beginning
    # using IF KEY IN DICT is much faster than IF DICT.GET(KEY)
    if (string, num) in memo:
    ...
    memo[(string, num)] = output
    return output

sum = 0

for line in lines:
    string = 4*(line.split(' ')[0] + '?') + line.split(' ')[0]
    num = 5*[int(x) for x in line.split(' ')[1].split(',')]
    
    sum += count(tuple(string), tuple(num))
    print(count(tuple(string), tuple(num)))
